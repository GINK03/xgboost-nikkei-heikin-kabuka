...
def step1():
  ...
  open("date_contexts.pkl", "wb").write(pickle.dumps(date_contexts)) 
...
def step3():
  date_kpi = {}
  with open("nikkei_mean.csv") as f:
    for line in f:
    ...
    open("date_kpi.pkl", "wb").write(pickle.dumps(date_kpi))
  print("正常に終了しました")


""" xgboostの形式に変換する, 予想も目的地も同一を使う """
def step4():
  date_kpi      = pickle.loads(open("date_kpi.pkl", "rb").read())
  date_contexts = pickle.loads(open("date_contexts.pkl", "rb").read())
  term_index    = pickle.loads(open("term_index.pkl", "rb").read())

  with open("xgboost.svm", "w") as f:
    for date, kpi in sorted(date_kpi.items(), key=lambda x:x[0]):
      print(date, kpi)
      id_freq = {}
      try:
        for term, freq in Counter(date_contexts[date].split()).items():
          id_freq[ term_index[term] ] = math.log(freq) # adhoc
...
""" トレインとテストに分割する """
def step5():
   sets = open("xgboost.svm", "r").read().split("\n")
   import random
   random.shuffle(sets)
   train = sets[:int(len(sets)*0.8)]
   test  = sets[int(len(sets)*0.8):]
   open("xgboost.svm.train", "w").write("\n".join(train))
   open("xgboost.svm.test", "w").write("\n".join(test))
